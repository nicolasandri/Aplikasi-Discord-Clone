...
export function ChannelList({ server, channels: _channels, selectedChannelId, onSelectChannel, onOpenSettings, onOpenServerSettings, onOpenUserSettings, onOpenInvite, onLeaveServer, isMobile = false, onClose, isOwner: propIsOwner, unreadCounts = {} }: ChannelListProps) {
  ...
  if (!server) {
    return (
      <div className="w-60 bg-[#111318] flex flex-col">
        ...
        {/* User Panel */}
        <div className="h-[52px] bg-[#0d0f13] px-2 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button className="p-2 text-gray-400 hover:text-white hover:bg-white/5 rounded">
  ...
  return (
    <div className="w-60 bg-[#111318] flex flex-col">
      {/* Server Header */}
      <div 
        className="h-12 px-4 flex items-center justify-between shadow-md cursor-pointer hover:bg-white/5 transition-colors group relative"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-1.5 rounded hover:bg-cyan-500/10 text-gray-400 hover:text-cyan-400 transition-colors opacity-0 group-hover:opacity-100"
            title="Invite People"
          >
        ...
        {showServerMenu && (
          <div className="absolute top-full left-0 right-0 mt-1 mx-2 bg-[#1a1d24] rounded-lg shadow-lg border border-white/5 z-50 overflow-hidden py-1">
            ...
            {/* Divider */}
            <div className="h-px bg-[#111318] mx-2 my-1" />
      ...
      <div className="flex-1 overflow-y-auto p-2">
              ...
              {canManage && (
                <div className="opacity-0 group-hover:opacity-100 flex items-center gap-1">
                  <button
                    ...
                    className="p-1 hover:bg-white/5 rounded text-gray-400 hover:text-white transition-opacity"
                    title="Buat Channel"
                  >
            ...
            <div className="mt-0.5">
              {uncategorizedChannels.map((channel) => {
                ...
                return (
                  <div
                    key={channel.id}
                    className={`group flex items-center gap-2 px-2 py-1.5 rounded-md mx-2 relative ${
                    ...
                    style={selectedChannelId === channel.id ? { marginLeft: '8px', paddingLeft: '6px' } : {}}
                  >
                    <button
                      onClick={() => {
        ...
        {canManage && (
          <button
            ...
            className="w-full flex items-center gap-2 px-2 py-2 mt-2 text-gray-400 hover:text-white hover:bg-white/5 rounded transition-colors"
          >
            <Plus className="w-4 h-4" />
      ...
      {/* User Panel */}
      <div className="h-[52px] bg-[#0d0f13] px-2 flex items-center justify-between relative">
        <div 
          onClick={() => setShowUserMenu(!showUserMenu)}
          className="flex items-center gap-2 px-2 py-1 rounded hover:bg-white/5 cursor-pointer flex-1"
        >
          <img
            src={user?.avatar?.startsWith('http') ? user?.avatar : `${BASE_URL}${user?.avatar}`}
            alt={user?.displayName || user?.username}
        ...
        {showUserMenu && (
          <div
            ref={userMenuRef}
            className="absolute bottom-full left-2 right-2 mb-2 bg-[#1a1d24] rounded-lg shadow-2xl overflow-hidden z-[100]"
          >
            {/* Compact User Info */}
            ...
            <div className="py-1">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-gray-400 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors text-left"
              >
